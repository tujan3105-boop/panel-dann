                ...
                <table class="table table-hover">
                    ...
                    <tbody>
                        <?php $__currentLoopData = $nodes; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $node): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <tr>
                                <td class="text-center text-muted left-icon" data-action="ping" data-location="<?php echo e(url('/admin/nodes/view/' . $node->id . '/system-information')); ?>"><i class="fa fa-fw fa-refresh fa-spin"></i></td>
                                <td><?php echo $node->maintenance_mode ? '<span class="label label-warning"><i class="fa fa-wrench"></i></span> ' : ''; ?><a href="<?php echo e(route('admin.nodes.view', $node->id)); ?>"><strong><?php echo e($node->name); ?></strong></a></td>
                                <td><?php echo e(optional($node->location)->short ?? '-'); ?></td>
                                <td><?php echo e($node->memory); ?> MiB</td>
                                <td><?php echo e($node->disk); ?> MiB</td>
                                <td class="text-center"><?php echo e($node->servers_count); ?></td>
                                <td class="text-center" style="color:<?php echo e(($node->scheme === 'https') ? '#00a65a' : '#dd4b39'); ?>"><i class="fa fa-<?php echo e(($node->scheme === 'https') ? 'lock' : 'unlock'); ?>"></i></td>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /var/gantengdann/resources/views/admin/nodes/index.blade.php ENDPATH**/ ?>