...
type resourceSpikeGuard struct {
	mu sync.Mutex

	server *Server

	cpuSuperConsecutive int
	lastActionAt        time.Time
}

func newResourceSpikeGuard(s *Server) *resourceSpikeGuard {
	return &resourceSpikeGuard{server: s}
}

func (r *resourceSpikeGuard) Observe(stats environment.Stats) {
	cfg := config.Get().System.Safety
	if !cfg.Enabled || !r.server.IsRunning() {
		r.mu.Lock()
		r.cpuSuperConsecutive = 0
		r.mu.Unlock()
		return
	}

	cpu := stats.CpuAbsolute
	cpuSuperByCores := cfg.CPUSuperCoresThresholdPercent > 0 && cpu >= cfg.CPUSuperCoresThresholdPercent
	cpuSuperByAllCores := cfg.CPUSuperAllCoresThresholdPercent > 0 && cpu >= cfg.CPUSuperAllCoresThresholdPercent
	cpuSuper := cpuSuperByCores || cpuSuperByAllCores

	r.mu.Lock()
	if cpuSuper {
		r.cpuSuperConsecutive++
	} else {
		r.cpuSuperConsecutive = 0
	}

	shouldAct := r.cpuSuperConsecutive >= cfg.CPUSuperConsecutiveCycles
	if shouldAct {
		cooldown := time.Duration(cfg.ActionCooldownSeconds) * time.Second
		if cooldown < 0 {
			cooldown = 0
		}
		if !r.lastActionAt.IsZero() && time.Since(r.lastActionAt) < cooldown {
			shouldAct = false
		} else {
	...
	go r.triggerCPUSuperAnomaly(
		cpu,
		cfg.CPUSuperCoresThresholdPercent,
		cfg.CPUSuperAllCoresThresholdPercent,
		cfg.CPUSuperConsecutiveCycles,
		cfg.StopTimeoutSeconds,
	)
}

func (r *resourceSpikeGuard) triggerCPUSuperAnomaly(
	cpu float64,
	cpuSuperCoresThreshold float64,
	cpuSuperAllCoresThreshold float64,
	cpuSuperConsecutiveThreshold int,
	stopTimeoutSeconds int,
) {
	s := r.server
	s.PublishConsoleOutputFromDaemon("Safety: sustained super-high CPU detected, stopping process now.")
	...
	if err := s.Environment.WaitForStop(s.Context(), timeout, true); err != nil {
		s.Log().WithField("error", err).Error("safety: failed to stop server after cpu super anomaly")
	}

	s.SaveActivity(
		s.NewRequestActivity("", "127.0.0.1"),
		ActivitySecurityCpuSuper,
		map[string]interface{}{
			"cpu_absolute":                     cpu,
			"cpu_super_cores_threshold":       cpuSuperCoresThreshold,
			"cpu_super_all_cores_threshold":   cpuSuperAllCoresThreshold,
			"cpu_super_consecutive_threshold": cpuSuperConsecutiveThreshold,
			"source":                           "wings_resource_guard",
			"auto_action":                      "stop_kill",
		},
	)
}
...
func (s *Server) StartEventListeners() {
	...
	spikeGuard := newResourceSpikeGuard(s)
